...
const resolveStatus = (totalAmount, alreadyPaid, owed) => {
    if (alreadyPaid <= 0)
        return "awaiting";
    if (alreadyPaid >= totalAmount || owed <= 0)
        return "paid";
...
salaryRouter.post("/records", async (req, res) => {
    ...
    const expectedToReceive = body.expectedToReceive === null || body.expectedToReceive === undefined || body.expectedToReceive === ""
        ? null
...
salaryRouter.post("/payouts", async (req, res) => {
    ...
    const salaryRecordId = body.salaryRecordId;
    const payout = await prisma.$transaction(async (tx) => {
        ...
        const manualExpected = record.expectedToReceive === null ? null : Math.max(Number(record.expectedToReceive) - amount, 0);
