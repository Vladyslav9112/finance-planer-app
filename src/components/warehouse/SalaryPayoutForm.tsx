import React, { useEffect } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { Banknote } from "lucide-react";
import { cn, today, formatCurrency } from "../../lib/utils";
import type { SalaryPayout } from "../../types";

const schema = z.object({
  source: z.string().min(1, "Джерело обов'язкове"),
  amount: z.coerce.number().positive("Сума має бути більше 0"),
  date: z.string().min(1, "Дата обов'язкова"),
  comment: z.string().optional(),
});

type FormValues = z.infer<typeof schema>;

interface SalaryPayoutFormProps {
  initial?: SalaryPayout;
  maxAmount?: number;
  onSubmit: (data: FormValues) => void;
  onCancel: () => void;
}

const inputClass =
  "w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-sm text-white placeholder-white/25 focus:outline-none focus:border-accent-lime/50 transition-colors";
const labelClass = "block text-xs font-600 text-white/45 mb-1.5";
const errorClass = "text-xs text-rose-400 mt-1";

export const SalaryPayoutForm: React.FC<SalaryPayoutFormProps> = ({
  initial,
  maxAmount,
  onSubmit,
  onCancel,
}) => {
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: initial
      ? {
          source: initial.source,
          amount: initial.amount,
          date: initial.date,
          comment: initial.comment,
        }
      : { date: today() },
  });

  useEffect(() => {
    if (initial) {
      reset({
        source: initial.source,
        amount: initial.amount,
        date: initial.date,
        comment: initial.comment,
      });
    }
  }, [initial, reset]);

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pb-2">
      {/* Max indicator */}
      {maxAmount !== undefined && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-accent-lime/[0.07] border border-accent-lime/20">
          <Banknote size={14} className="text-accent-lime flex-shrink-0" />
          <p className="text-xs text-accent-lime font-500">
            Доступно до видачі: <strong>{formatCurrency(maxAmount)}</strong>
          </p>
        </div>
      )}

      {/* Source */}
      <div>
        <label className={labelClass}>Джерело / Назва *</label>
        <input
          {...register("source")}
          placeholder="Підприємство або ФОП..."
          className={cn(inputClass, errors.source && "border-rose-500/50")}
        />
        {errors.source && <p className={errorClass}>{errors.source.message}</p>}
      </div>

      {/* Amount */}
      <div>
        <label className={labelClass}>Сума виплати (₴) *</label>
        <input
          type="number"
          step="0.01"
          min="0"
          max={maxAmount}
          {...register("amount")}
          placeholder="0.00"
          className={cn(inputClass, errors.amount && "border-rose-500/50")}
        />
        {errors.amount && <p className={errorClass}>{errors.amount.message}</p>}
      </div>

      {/* Date */}
      <div>
        <label className={labelClass}>Дата виплати *</label>
        <input
          type="date"
          {...register("date")}
          className={cn(inputClass, errors.date && "border-rose-500/50")}
        />
        {errors.date && <p className={errorClass}>{errors.date.message}</p>}
      </div>

      {/* Comment */}
      <div>
        <label className={labelClass}>Коментар</label>
        <textarea
          {...register("comment")}
          placeholder="Необов'язкова нотатка..."
          rows={2}
          className={cn(inputClass, "resize-none")}
        />
      </div>

      {/* Buttons */}
      <div className="flex gap-3 pt-2">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 py-3 rounded-xl border border-white/[0.08] text-white/50 text-sm font-600 hover:border-white/15 hover:text-white/70 transition-colors"
        >
          Скасувати
        </button>
        <button
          type="submit"
          className="flex-1 py-3 rounded-xl bg-accent-lime/20 border border-accent-lime/40 text-accent-lime text-sm font-700 hover:bg-accent-lime/30 transition-all flex items-center justify-center gap-2"
        >
          <Banknote size={15} />
          {initial ? "Зберегти" : "Видати ЗП"}
        </button>
      </div>
    </form>
  );
};
